...
public class ArraysLesson {
  public static void main(String args[]) {
    ...
    // The elements of an array are indexed, which means are accessed with numbers
    // (called indices).
    // Arrays are used when we need to collect data that are related to each other
    ...
    // To initialize an array, indicated the number or elements or the actual
    // elements themselves.
    ...
    // Assigning values to the array can be done using the assignment operator.
    // The index must be provided for the array to know where to assign the value.
    // Arrays start with index = 0 and the last element is at index n-1 where n is
    ...
    // The issue with arrays in Java is that it is of fixed length, meaning after
    // definition, there is no way to add elements in the array.
    // Hence ArrayLists are used to add versatility to the array.
    // ArrayLists are part of Java's utilities package, so if ArrayLists are to be
    // used, the java.utils.* must be imported.
    ...
    // To access individual elements in the array list, the .get() and .set()
    // methods are used.
